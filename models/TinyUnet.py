...
class UNetTiny(nn.Module):
    ...
    def forward(self, x):
        ...
        center = self.center(e2) #; print('center:', center.size())
        ...
        d2 = self.decoder2(center, e2) #; print('d2:', d2.size())
...
if __name__=='__main__':
    # image=torch.from_numpy(np.zeros(shape=(1,3,128,128),dtype = np.float32))
    # mask=torch.from_numpy(np.zeros(shape = (1,1, 128, 128), dtype = np.int))
    UnetTiny=UNetTiny(50,3,)
    # UnetTiny.train()
    # predict=UnetTiny(image)
    # print(predict.shape)
    summary(UnetTiny,(3,512,512),1,'cpu')