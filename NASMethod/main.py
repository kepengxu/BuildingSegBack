...
from preporcess.CrowdaiData import GetDataloaderSearch as CrowdGen
import os
sys.path.append('..')
from NASMethod.util.loss.loss import SegmentationLosses
from NASMethod.util.datasets import get_dataset, datasets
from NASMethod.util.utils import get_logger, save_checkpoint
from NASMethod.util.utils import average_meter, calc_time
...
from NASMethod.search.backbone.nas_unet_search import NasUnetSearch, Architecture
import socket
...
class SearchNetwork(object):
    ...
    def _init_logger(self):
        log_dir = '/Disk4/xkp/project/BuildingSegBackup/LOG/nasunet/search' + '/{}'.format(self.cfg['data']['dataset']) +\
    ...
    def _init_device(self):
        self.device = torch.device("cuda" if self.cfg['searching']['gpu'] and torch.cuda.is_available() else "cpu")
        print(self.device)
        np.random.seed(self.cfg.get('seed', 1337))
        torch.manual_seed(self.cfg.get('seed', 1337))
        if self.cfg['searching']['gpu']:
    ...
    def _init_dataset(self):
        ...
        self.train_queue,self.valid_queue,_ =CrowdGen(trainimagepath=self.cfg['data']['trainimagepath'],
                                                       ...
                                                       batchsize=self.cfg['searching']['batch_size'],
                                                       numworkers=1)
    ...
    def _init_model(self):
        ...
        # Setup loss function
        self.criterion=LOSSDICT[self.cfg['searching']['loss']['name']]().to(self.device)
    ...
    def train(self):
        ...
        self.model.train()
        print('************************** is gen alpha  train the alpha ',self.epoch >= self.cfg['searching']['alpha_begin'])
        ...
        for step, (input, target) in enumerate(tbar):
            ...
            # for i in range(input_valid.shape[0]):
            #     image=np.moveaxis(input_valid[i].numpy(),0,-1)
            #     plt.imshow(image)
            #     plt.show()
            #     ta=target_valid[i,0,:,:].numpy()
            #     plt.imshow(ta)
            #     plt.show()
            ...
            if step % self.cfg['searching']['report_freq'] == 0:
                ...
                self.logger.info('pixAcc: %.3f; mIoU: %.5f ; f1 score : %.5f  ' % (pixAcc, mIoU,f1s))
    ...
    def infer(self):
        ...
        with torch.no_grad():
            for step, (input, target) in enumerate(tbar):
                ...
                if step % self.cfg['searching']['report_freq'] == 0:
                    mIoU,_,pixAcc,_ = self.metric_val.get()
        ...
        mIoU,iou,pixAcc,f1sc= self.metric_val.get()
        ...
        self.writer.add_scalar('Val/f1', f1sc, self.epoch)
