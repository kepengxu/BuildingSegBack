...
from preporcess.CrowdaiData import GetDataloaderTrain as CrowdGen
from multiprocessing import cpu_count
sys.path.append('..')
from NASMethod.util.loss.loss import SegmentationLosses
from  NASMethod.util.datasets import get_dataset
from  NASMethod.util.utils import get_logger, save_checkpoint, calc_time, store_images
from  NASMethod.util.utils import  average_meter, weights_init
from  NASMethod.util.utils import get_gpus_memory_info, calc_parameters_count
from  NASMethod.util.schedulers import get_scheduler
from  NASMethod.util.optimizers import get_optimizer
from  NASMethod.util.challenge.promise12.store_test_seg import predict_test
from  NASMethod.util.metrics import *
from NASMethod.models import get_segmentation_model
import NASMethod.models.geno_searched as geno_types
from tensorboardX import SummaryWriter
from utils.losses import LOSSDICT
...
class Network(object):
    ...
    def _init_configure(self):
        ...
        parser.add_argument('--config',nargs='?',type=str,default='/Disk4/xkp/project/BuildingSegBackup/NASMethod/configs/buildingSegconfig/unet_nas_building1.yml',
    ...
    def _init_logger(self):
        log_dir = '/Disk4/xkp/project/BuildingSegBackup/LOG/nasunet/train' + '/{}'.format(self.cfg['data']['dataset']) \
    ...
    def _init_dataset(self):
        numwork=int(cpu_count()//2)
        print(self.cfg['searching']['batch_size'])
        self.train_queue,self.valid_queue =CrowdGen(trainimagepath=self.cfg['data']['trainimagepath'],
                                                       trainmaskpath=self.cfg['data']['trainmaskpath'],
                                                       valimagepath=self.cfg['data']['valimagepath'],
                                                       valmaskpath=self.cfg['data']['valmaskpath'],
                                                       shape=self.cfg['data']['shape'],
                                                       padshape=self.cfg['data']['padshape'],
                                                       batchsize=self.cfg['training']['batch_size'],
                                                       numworkers=1)
        print('the loader length is :!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        print(self.train_queue.__len__(),self.valid_queue.__len__(),numwork)
        self.in_channels=3
        self.n_classes=1
    def _init_model(self):

        # Setup loss function
        self.criterion=LOSSDICT[self.cfg['training']['loss']['name']]().to(self.device)
    ...
    def train(self):
        ...
        for step, (input, target) in enumerate(tbar):
            ...
            if step % self.cfg['training']['report_freq'] == 0:
                ...
                mIoU,iou,pixAcc,f1s= self.metric_train.get()

                self.logger.info('pixAcc: {},  mIoU: {}'.format(pixAcc, mIoU))
                tbar.set_description('train loss: %.6f; pixAcc: %.3f; mIoU %.6f;  iou : %.6f; f1 score: %.6f' % (self.train_loss_meter.mloss, pixAcc, mIoU,iou,f1s))
        ...
        self.writer.add_scalar('Train/iou',iou , self.epoch)
        self.writer.add_scalar('Train/f1 score', f1s, self.epoch)
    ...
    def val(self):
        ...
        with torch.no_grad():
            for step, (input, target) in enumerate(tbar):
                ...
                if step % self.cfg['training']['report_freq'] == 0:
                    ...
                    self.logger.info('val loss: {}, pixAcc: {}, mIoU: {}, f1 score: {}'.format(
                        self.val_loss_meter.mloss, pixAcc, mIoU,f1s))
                    tbar.set_description('val loss: %.6f, pixAcc: %.3f, mIoU: %.6f , f1 score: %.6f'
                                         % (self.val_loss_meter.mloss, pixAcc, mIoU,f1s))
        ...
        # save in tensorboard scalars
        mIoU, iou, pixAcc, f1s = self.metric_val.get()
        ...
        self.writer.add_scalar('Val/iou', iou, self.epoch)
        self.writer.add_scalar('Val/f1score', f1s, self.epoch)
