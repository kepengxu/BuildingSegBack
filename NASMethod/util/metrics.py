...
from NASMethod.util.Mertics import IoU,F1score,Accuracy
from NASMethod.util.Mertics import iout as IOUT
class SegmentationMetric(object):
    ...
    def update(self, labels, preds):
        ...
        def evaluate_worker(self, label, pred):
            ...
            timeiou=IoU(pred,label)
            timeiout=IOUT(pred,label)
            tacc=Accuracy(pred,label)
            timeacc=tacc
            timef1= F1score(pred,label)

            # label=label.numpy()
            # pred=pred.numpy()
            # 
            # correct, labeled = pixel_accuracy(
            #     pred, label)
            # 
            ...
            with self.lock:
                ...
                self.Num += 1
                self.iou+=np.mean(timeiou)
                self.iout +=np.mean(timeiout)
                self.Acc +=timeacc
                self.f1 +=timef1
            return
    ...
    def get(self):
        ...
        # return pixAcc, mIoU
        outiout=self.iout/self.Num
        outiou=self.iou/self.Num
        outacc=self.Acc/self.Num
        outf1=self.f1/self.Num
        return outiout,outiou,outacc,outf1
    def reset(self):
        # self.total_inter = 0
        # self.total_union = 0
...
def batch_pix_accuracy(output, target):
    ...
    print('shape pre',predict.shape,' shape target',target.shape)
...
def batch_intersection_union(output, target, nclass):
    ...
    """
    # TODO:
    # need to debug and know the predict = torch.max(output, 1)[1]  work
    predict = torch.max(output, 1)[1]
